    ...
    export class Dispatcher {
        ...
        public completeContextSwitch() {
            ...
            if (this.contextSwitching) {
                ...
                let prevPID: number = _PCB.runningPID;
                _PCB.updateRunning();
                ...
                if (_PCB.processes[_PCB.runningPID].Location === "Disk" && _PCB.processes[_PCB.runningPID].Status != "Terminated"){
                    let rollOutProcess: string = _MA.readWholeProgram(_PCB.processes[prevPID].Segment); //gets the whole program as a string
        ...
        public rollOut(process: string, pid: number): number{
        ...
        public rollIn(pid: number, segment: number){
            console.log(`Rolling in PID: ${pid} to segment: ${segment}`);
            
            let fileName: string = `program${pid}.stud`;

            //write file to the memory
            let processArr: string[] = Utils.splitEveryOther(_krnDiskDriver.read(fileName));
